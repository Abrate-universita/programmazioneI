...
int somma_dispariR(int n){
	if(n == 0)
		...
		if(n % 2 != 0) return n + somma_dispariR(n - 1);
		else return somma_dispariR(n - 1);
	}
}
...
int prodotto_multipli_in_intervalloR(int n_min, int n, int k){
	if(n < n_min)
		...
		if(n % k == 0) return n * prodotto_multipli_in_intervalloR(n_min, n - 1, k);
		else return prodotto_multipli_in_intervalloR(n_min, n - 1, k);
	}	
}
