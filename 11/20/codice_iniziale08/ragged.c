...
int main(void) {
    ...
    for (size_t r=0; r<ROWS; r++) {
        printf("riga %Iu: elementi=%Iu  somma=%-2d  (atteso %-2d)\n", 
...
void sostituisci_se_esiste_primo(int mat[ROWS][COLS], const size_t rags[ROWS], const int val){
	...
	for(size_t i = 0; i < ROWS; i++){
		for(size_t j = 0; j < rags[i] && !primo; j++){
			primo = verifica_primalita(mat[i][j]);
		}
		for(size_t j = 0; j < rags[i] && primo; j++)
			mat[i][j] = val;
		primo = false;
