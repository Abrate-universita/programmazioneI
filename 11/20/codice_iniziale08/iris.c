...
int main(void) {
    ...
    for (size_t i=0; i<ROWS; i++) {
        printf("%-20s %2Iu    %.2f   (min: %.2f max: %.2f)\n",
               specie[i], rags[i],
...
void minmax_riga(const float mat[ROWS][COLS], const size_t rags[ROWS], const size_t r, float *pMin, float* pMax){
	...
	*pMax = -100;
	for(size_t i = 0; i < rags[r]; i++){
		if(mat[r][i] > *pMax) *pMax = mat[r][i];
		if(mat[r][i] < *pMin) *pMin = mat[r][i];
	}
}
...
void filtra_riga_minmax(float mat[ROWS][COLS],  size_t rags[ROWS], const size_t r, float min, float max){
	// creo l'indice che sostituirà quello attuale di rags[r]
	size_t pos = 0;
	
	// creo un vettore di supporto (riga r della matrice) che conterrà solo i valori validi
	float riga[COLS];
	
	for(size_t i = 0; i < rags[r]; i++){
		// controllo l'elemento in posizione i è valido
	...
	// ora modifico tutta la riga della matrice inserendo solo i valori validi
	rags[r] = pos;
	for(size_t i = 0; i < rags[r]; i++)
		mat[r][i] = riga[i];
}
